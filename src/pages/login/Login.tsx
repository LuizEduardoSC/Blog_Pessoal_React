    ...
    return (
        <>
            <div className={`grid grid-cols-1 lg:grid-cols-2 h-screen place-items-center font-bold transition-colors duration-300 relative ${theme === 'dark' ? 'bg-slate-900' : 'bg-indigo-900'}`}>
                <button 
                    onClick={toggleTheme} 
                    className="absolute bottom-10 right-10 lg:right-[52%] lg:left-auto z-50 w-14 h-14 rounded-full bg-white text-indigo-900 dark:bg-indigo-600 dark:text-white shadow-2xl hover:scale-110 active:scale-95 transition-all flex items-center justify-center border-2 border-indigo-900 dark:border-slate-800"
                    title="Alternar tema"
                >
